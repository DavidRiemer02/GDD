...
import de.unimarburg.generateddatasetdetector.utils.MetadataUtils;
...
public class MetanomeImpl {
    ...
    private static List<UniqueColumnCombination> executeHyUCC(List<Table> tables) {
        ...
        try {
            for (Table table : tables) {
                ...
                //if (Metanome.SAVE)
                    //MetadataUtils.saveUCCs(MetadataUtils.getMetadataRootPathFromTable(Path.of(tables.get(0).getPath())), ucCs);
            }
    ...
    private static List<InclusionDependency> executeBinder(List<Table> tables) {
        try {
            ...
            //if (Metanome.SAVE)
                //MetadataUtils.saveINDs(MetadataUtils.getMetadataRootPathFromTable(Path.of(tables.get(0).getPath())), inclusionDependencies);
    ...
    private static List<FunctionalDependency> executeHyFD(List<Table> tables) {
        ...
        try {
            for (Table table : tables) {
                ...
                //if (Metanome.SAVE)
                    //MetadataUtils.saveFDs(MetadataUtils.getMetadataRootPathFromTable(Path.of(tables.get(0).getPath())), fDs);
            }
        } catch (AlgorithmExecutionException | FileNotFoundException e) {
            e.printStackTrace();
        }
