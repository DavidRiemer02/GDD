...
class UMAPColumnEmbedder:
    def __init__(self, model_dir="models/umapRandomForest", n_components=3, aggregation="mean+std+min"):
    ...
    def extract_per_column_features(self, numeric_df):
        ...
        for col in numeric_df.columns:
            ...
            if len(col_data) < 3 or col_data.nunique() < 2:
                continue
            try:
                features = [
                    col_data.mean(),
    ...
    def fit_umap(self, list_of_column_feature_arrays):
        ...
        self.umap_model = umap.UMAP(n_components=self.n_components, n_neighbors=15, min_dist=0.1, metric='manhattan', random_state=42).fit(X_scaled)
    ...
    def transform_and_aggregate(self, numeric_df):
        ...
        if col_feat.shape[0] == 0:
            return np.zeros(self.n_components * 3 if "mean+std+min" in self.aggregation else self.n_components)
        ...
        elif self.aggregation == "mean+std+min":
            agg = np.concatenate([
                embedded.mean(axis=0),
                embedded.std(axis=0),
                embedded.min(axis=0)
            ])
            return agg
